...
public:
    ...
    int lowerBound(vector<int> &subseq, int target)
    {
        int start = 0;
    ...
    int solve(int a[], int n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            int insertionPoint = lowerBound(subseq, a[i]);

            if (insertionPoint >= subseq.size())
            {
                subseq.push_back(a[i]);
            }
            else
            {
                subseq[insertionPoint] = a[i];
            }
        }
